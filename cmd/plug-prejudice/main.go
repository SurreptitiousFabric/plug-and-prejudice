...
func run() int {
	...
	coverage := analyze.SummarizeCoverage(result.Files, result.Contents, limitations, result.Errors)
	status := report.StatusComplete
	if len(analysis.Unknowns) > 0 || len(result.Errors) > 0 || len(limitations) > 0 {
