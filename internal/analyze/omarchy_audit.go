...
func OmarchyAuditEvidenceInput(audit omarchyaudit.Report) report.EvidenceInput {
	return report.EvidenceInput{ID: OmarchyAuditEvidenceInputID, Type: report.EvidenceInputOmarchyAudit, Label: "pinned Omarchy audit for " + audit.ID, Format: report.OmarchyAuditInputFormat, Version: report.OmarchyAuditInputVersion}
}
...
func IngestOmarchyAudit(audit omarchyaudit.Report, result *Result) []report.Comparison {
	...
	for keyIndex, key := range keys {
		if keyIndex >= maxOmarchyAuditComparisons {
			...
			budgetProvenance.RuleID = "omarchy-audit-comparison-budget/v1"
		...
		if ownOK && otherOK {
			comparisons = append(comparisons, report.Comparison{Type: report.RelationshipCorroborates, FromKind: own.kind, FromID: own.id, ToKind: other.kind, ToID: other.id, Basis: report.ComparisonBasis{Kind: string(own.kind), Subject: key}})
			continue
		}
		...
		if otherOK {
			...
			findingProvenance.RuleID = report.CoverageComparisonRule
			...
			observed = own
			findingProvenance = sourceProvenance(report.CoverageComparisonRule)
		...
		finding := report.Finding{ID: "finding-omarchy-coverage-disagreement-" + stablePathID(key), Claim: report.ClaimFact, Severity: report.SeverityInformational, Confidence: report.ConfidenceHigh, Category: report.CoverageDifferenceCategory, Scope: report.ScopeUnknown, Title: "Independent and Omarchy audit observations differ", Explanation: explanation, Evidence: []report.Evidence{observed.evidence}, Provenance: findingProvenance}
		if appendFinding(result, finding) {
			comparisons = append(comparisons, report.Comparison{Type: report.RelationshipDisagreesWith, FromKind: observed.kind, FromID: observed.id, ToKind: report.NodeFinding, ToID: finding.ID, Basis: report.ComparisonBasis{Kind: "coverage", Subject: key}})
		}
	}
	return comparisons
}
