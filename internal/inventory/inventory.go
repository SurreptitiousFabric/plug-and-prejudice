...
func Scan(target string, limits Limits) (Result, error) {
	...
	w := &walker{limits: limits, result: Result{Contents: make(map[string][]byte)}, rootMountID: mountID}
	...
	for _, file := range w.result.Files {
		for _, value := range []string{file.Path, file.Kind, file.Mode, fmt.Sprint(file.Size), file.SHA256, file.LinkTarget, file.SkipReason} {
			_, _ = digest.Write([]byte(value))
			_, _ = digest.Write([]byte{0})
		}
	}
	w.result.RootDigest = hex.EncodeToString(digest.Sum(nil))
	return w.result, nil
}
...
func (w *walker) inspectRegular(root *os.File, name string, expected fs.FileInfo, out *report.File) {
	...
	if !stableRead(f, expected, int64(len(data))) {
		w.scanError("changed-during-scan", "file identity, size, or timestamps changed while it was read", out.Path)
		return
	}
...
func (w *walker) inspectELF(f *os.File, expected fs.FileInfo, out *report.File) {
	...
	if !stableRead(f, expected, int64(len(data))) {
		w.scanError("changed-during-scan", "ELF identity, size, or timestamps changed while it was read", out.Path)
		return
	}
