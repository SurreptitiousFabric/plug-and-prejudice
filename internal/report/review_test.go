...
func TestReviewSummarySeparatesImpactConfidenceCoverageUnknownsAndCounts(t *testing.T) {
	...
	if review.EvidenceConfidence.Level != "medium" || review.EvidenceConfidence.High != 5 || review.EvidenceConfidence.Medium != 1 || review.EvidenceConfidence.Low != 0 {
	...
	if review.Counts != (ClaimCounts{Facts: 3, Inferences: 1, UnknownBehaviors: 2}) {
...
func TestValidateRejectsReviewSummaryThatDoesNotMatchEvidence(t *testing.T) {
	r := graphReport(t)
	if err := r.BuildReviewSummary(NewCoverageSummary(1, 0, 0)); err != nil {
		t.Fatal(err)
	}
