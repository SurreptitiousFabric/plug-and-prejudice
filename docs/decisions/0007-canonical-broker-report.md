...
root-digest presence, and resource-policy checks, encode the typed Go value into
a fresh buffer using the standard library JSON encoder. Emit only those new
...
approved, the implementation may share one small tested bounded-encoding
primitive; the two checks and their failure messages remain distinct trust
boundaries.
...
1. a report containing duplicate member names is either rejected during strict
   ...
   broker re-encodes;
2. escaped lone-surrogate, invalid UTF-8, HTML-sensitive, C0/C1, and bidi string
   cases prove QML-bound bytes represent the validated Go strings;
3. exact 16 MiB and first-byte-over-limit tests prove no partial stdout;
...
- Scanner indentation and member order are no longer a UI protocol guarantee.
