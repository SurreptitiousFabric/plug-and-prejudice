...
- Obtains a bounded list of valid installed directory IDs from the trusted
  broker only after resource containment is verified, without reading
  plugin-authored manifests merely to populate the UI.
...
- Re-enters a randomized, verified systemd user scope before listing or resolving
  targets, verifies its runtime, then applies process rlimits and constructs
  Bubblewrap containment. The outer broker performs bounded whole-scope teardown.
